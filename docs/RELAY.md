...
`WEBRTC_STUN_PUBLIC_PORT` 是宿主机映射端口，`WEBRTC_STUN_PORT` 是容器内监听端口；修改端口时需要同步修改这两个 environment 值和 `ports` 中的 UDP 映射。`WEBRTC_STUN_PUBLIC_HOST` 留空时，relay 会从网页请求的 Host 自动生成候选地址。当前默认只使用 relay 自己的 STUN-only 服务；只有明确通过环境变量 `WEBRTC_STUN_SERVERS` 增加地址时，才会额外使用外部 STUN 服务。
